...
static struct token* token_make_string(char start_delim, char end_delim)
{
    ...
    for(; c!= end_delim && c!= EOF; c = nextc())
    {
        if (c == '\\')
        {
            //handling escape character like \n
            continue; 
        }
...
static void lex_finish_expression()
{
    ...
    {
        compiler_error(lex_process->compiler, "Closed an expression which was never opened\n");
    }
}
...
struct token* token_make_multiline_comment()
{
    ...
    while(1)
    {
        ...
        if (c == EOF)
        {
            compiler_error(lex_process->compiler, "you did now close this comment\n");
        }
        else if (c == '*')
        {
            //SKIP *
...
struct token* read_next_token()
{
    ...
    switch(c)
    {
        ...
        //we dont care for whitespace or tab
        case ' ':
        case '\t':
